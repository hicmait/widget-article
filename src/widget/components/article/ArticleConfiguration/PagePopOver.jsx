...
import { setArticle, savePage } from "../../../redux/actions";
import _ from "../../../i18n";
...
export default function ThemePopOver(props) {
  ...
  const [pageTitleNl, setPageTitleNl] = useState("");
  ...
  const selectedPages = useSelector((state) => state.articles.article.pages);
  ...
  const validate = () => {
    ...
    if (
      pageTitleFr.trim().length === 0 ||
      pageTitleNl.trim().length === 0 ||
      pageTitleEn.trim().length === 0
    ) {
      errors.push(_("article.validate_page_title"));
    }
