...
const QuoteEmbed = {
  ...
  setSubmenu: function (core) {
    ...
    listDiv.innerHTML = `
      ...
      <textarea class="se-input-form" type="text" style="padding: 5px;border: 1px solid #CCC; width: 100%" placeholder="${_(
        "article.quote_text"
      )}" ></textarea>
      </div>
      <div class="se-form-group">
      <input class="se-input-form" name="author" type="text" placeholder="${_(
        "article.quote_author"
      )}" />
      </div>
      <div class="se-form-group">
        <button type="button" class="se-plugin-btn se-tooltip">${_(
          "article.quote_add"
        )}</button>
      </div>
      </div>`;
  ...
  onClick: function () {
    ...
    const authorValue =
      this.context.quoteSubmenu.authorTextElement.value.trim();
    if (!value) return;
